...
| Measured, not promoted | Phase D distributional baselines — originally reported as CRPS better in 20/20 cells; **re-measured 2026-09-26 as 18/20, 15/20 and 14/20 at about half the effect size** (`docs/measured_results.md` §6.11). Under a block bootstrap respecting overlapping labels and cross-cell correlation, `regime_conditional` and `regularized_linear` reach p ≈ 0.014 and survive BH at the model level (§6.12, §6.13). This is **robust retrospective evidence, not confirmatory** — the test was chosen after the result was seen — and no downstream turbo net-EV test has been run |
