...
for it -- ``test_scan_ev.py``'s
``test_run_scan_with_rng_writes_forecasts_and_ledger_and_can_reach_actionable``
-- branches on ``if candidate.category == Category.ACTIONABLE`` and
currently takes the ``else`` branch, so the email path has never actually
run under test. ``ranking/gates.py::evaluate_gates`` is already verified
correct elsewhere -- what is missing is coverage of the step *after* the
gate: does an ACTIONABLE candidate reliably become a sent email, get
deduplicated correctly, and get a warning (never a silent drop) if its
pricing/evaluation data has gone missing by delivery time.
...
from turboedge.pipeline.scan import _maybe_send_trade_proposals, _PricedProduct
